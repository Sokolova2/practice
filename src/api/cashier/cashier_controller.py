...
async def get_product(db: AsyncSession = Depends(get_session_app)):
    service = CashierService(db)
    return await service.get_product()

@cashier_routes.post("/create/order", summary="Create order")
async def add_product(order: OrdersAddSchemas, db: AsyncSession = Depends(get_session_app)):
    service = CashierService(db)
...
async def check(id_order: int, db: AsyncSession = Depends(get_session_app)):
    service = CashierService(db)
